...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(xhxUserService).passwordEncoder(new BCryptPasswordEncoder());
    }

    @Override
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/index.html", "/static/**","/login_p","/checkPhone","/regSave","/riskMonitor/findAllRiskMonitor");
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
       http.authorizeRequests()
            ...
            public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {
                httpServletResponse.setContentType("application/json;charset=utf-8");
                String ipAddress = HTTPUtils.getIpAddress(httpServletRequest);//获取登陆用户的IP
                String username = httpServletRequest.getParameter("username");
                int i = xhxUserService.updateXhxUserIp(ipAddress,username);
                PrintWriter out = httpServletResponse.getWriter();
                if(i<1){
                    String s = "{\"status\":\"success\",\"msg\":更新用户失败}";
                    out.write(s);
                    out.flush();
                    out.close();
                }
